...
export function registerRecommendMealsTool(server: McpServer, recipes: Recipe[]) {
  server.tool(
    "recommendMeals",
    "智能规划一周膳食。根据用餐人数、忌口食材和过敏原，生成一周的早中晚餐计划，并提供购物清单。",
    {
      peopleCount: z.number().int().min(1).max(10)
                   .describe('用餐人数（1-10人）'),
      allergies: z.array(z.string()).optional()
                 .describe('过敏原列表，如 ["虾", "花生", "牛奶"]，可选'),
      avoidItems: z.array(z.string()).optional()
                  .describe('忌口食材列表，如 ["香菜", "葱", "姜"]，可选')
    },
    async ({ peopleCount, allergies = [], avoidItems = [] }: { 
      peopleCount: number,
      allergies?: string[], 
      avoidItems?: string[]
    }) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j < breakfastCount && recipesByCategory['早餐']?.length > 0; j++) {
        ...
        for (let j = 0; j < mealCount; j++) {
          ...
          while (!recipesByCategory[selectedCategory]?.length) {
            selectedCategory = categories[Math.floor(Math.random() * categories.length)];
            if (categories.every(cat => !recipesByCategory[cat]?.length)) break;
          }
          
          if (recipesByCategory[selectedCategory]?.length > 0) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const getMeals = (count: number): SimpleRecipe[] => {
          ...
          for (let j = 0; j < count; j++) {
            ...
            } else if (recipesByCategory['主食']?.length > 0) {
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify({
              summary: `已为${peopleCount}人生成一周膳食计划`,
              excludedAllergies: allergies.length > 0 ? allergies : '无',
              excludedItems: avoidItems.length > 0 ? avoidItems : '无',
              plan: mealPlan
            }, null, 2),
          },
        ],
